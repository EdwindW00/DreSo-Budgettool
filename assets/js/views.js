...
function renderBudget(project) {
  ...
  const catBlocks = Store.state.categories.map(cat => {
    ...
    const subBlocks = subheaders.map(sub => {
      ...
      const usedCodes = new Set(lines.filter(l => l.code).map(l => l.code));
      const availableCatalog = Store.state.catalog.filter(i => i.subheader === sub.code && !usedCodes.has(i.code));
      ...
      const lineRows = lines.map(line => {
        ...
        return `
          <tr class="item-row">
            ...
            <td>
              ${catalogItem
                ? `${escapeHTML(line.description)} <span class="chip">catalogus</span>`
                : `<input class="desc-input" data-line-field="description" data-line-id="${line.id}" value="${escapeHTML(line.description)}">`}
            </td>
            <td class="center">
            ...
            <td class="center">
              ${catalogItem
                ? `<span class="unit-badge">${escapeHTML(line.unit)}</span>`
                : `<input class="qty-input" style="width:70px" data-line-field="unit" data-line-id="${line.id}" value="${escapeHTML(line.unit)}">`}
            </td>
            <td class="num">
            ...
            <td class="center"><button class="icon-btn" data-action="remove-line" data-line-id="${line.id}" title="Verwijderen">&#10005;</button></td>
          </tr>`;
      }).join("");

      const catalogOptions = availableCatalog.map(i => `<option value="${i.code}">${i.code} &middot; ${escapeHTML(i.description)}</option>`).join("");
      ...
      return `
        ...
        <tr class="add-row">
          <td></td>
          <td colspan="2">
            <select class="add-select" style="width:100%;">
              <option value="">+ item toevoegen uit catalogus…</option>
              ${catalogOptions}
              <option value="__custom__">&#10010; Aangepast item…</option>
            </select>
          </td>
          <td colspan="2" class="custom-fields hidden" style="display:flex; gap:6px;">
            <input class="add-custom-desc" placeholder="Omschrijving" style="flex:1;">
            <input class="add-custom-unit" placeholder="Eenheid" style="width:70px;">
            <input class="add-custom-price" type="number" placeholder="€/stuk" style="width:80px;">
          </td>
  ...
  el.innerHTML = `
    <div class="view-header">
      <div>
        ...
        <p>Vul hoeveelheden in per regel. Eenheidsprijzen komen uit de catalogus en schalen automatisch mee met het afwerkingsniveau
        (<strong>${FINISHING_LEVELS[project.finishingLevel].label} ${FINISHING_LEVELS[project.finishingLevel].suffix}</strong>).
        Pas een prijs hier aan om alleen deze regel te overschrijven.</p>
      </div>
      <div class="view-actions">
  ...
  const cards = projects.map(p => {
    ...
    return `
      <div class="project-card ${isActive ? "active" : ""}">
        <div>
          <div class="pname">${escapeHTML(p.name)} ${isActive ? '<span class="chip">actief</span>' : ""}</div>
...
function renderReport(project) {
  ...
  const detailSections = Store.state.categories.map((cat, idx) => {
    const subheaders = Store.subheadersFor(cat.code);
    const hasLines = project.lines.some(l => {
    ...
    const subTables = subheaders.map(sub => {
      ...
      const rows = lines.map(l => `
        <tr>
          ...
          <td class="num">${fmtNum(l.quantity)} ${escapeHTML(l.unit)}</td>
